...
class GWorkerMng extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
...
/*       headerRight:(
  ...
  alterManager(){
    fetchRequest('/group/doGroupManager', 'POST', {id: this.state.id}).then(res=>{
      if (res.code === 200) {
        ToastNative('设置成功')
      } else {
  ...
  deleteWorker(){
    fetchRequest('/group/deleteWorkerByGroup', 'POST', {id: this.state.id}).then(res=>{
      if (res.code === 200) {
      ...
      <View style={styles.wrap}>
        <View style={styles.box}>
          <ImageBackground style={styles.bg} source={require('../../../images/worker.png')}>
            ...
            <Text>班组: {this.state.groups.join('、')}</Text>
          </ImageBackground>
        </View>  
        <View>
